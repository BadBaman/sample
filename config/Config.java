...
public class Config {
	...
	private boolean runningFabricCATLS = false;
	private boolean runningFabricTLS = false;
	...
	private Config() {
		...
		try {
			...
			for (int i = 0; i < split.length; i++) {
				...
				try {
					enrollOrg = Util.getEnrollment(Path + admin_pk, pkFiles[0].getName(), Path + admin_cert,
							certFiles[0].getName());
