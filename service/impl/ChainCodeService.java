	...
	public CommonResult instantiateChaincode( String belongWithOrg,String[] peerWithOrgs,  String channelName){
		...
		try {
			...
			for (int i = 0; i < peerWithOrgs.length; i++) {
				Chaincode orgcode = codeService.queryChaincodeIsExist(peerWithOrgs[i], channelName);
				if(orgcode==null) {
					return CommonResult.failed("some organization have not  installed chaincode in the channel " + channelName);
				}
			}
			if (code == null) {
			...
			//是否已经实例化 检测
			System.out.println(!(code.getIsInstantiated()).equals("false"));
...
//        transactionProposalRequest.setChaincodeEndorsementPolicy(chaincodeEndorsementPolicy);
			...
			log.info("Sending chaincode transaction " + code.getChaincodeName() + "_" + chaincodeFunction + " to orderer.");
...
//			}
			...
			for (int i = 0; i < peerWithOrgs.length; i++) {
				Chaincode orgcode = codeService.queryChaincodeIsExist(peerWithOrgs[i], channelName);
				if((orgcode.getIsInstantiated()).equals("false")) {
					return CommonResult.failed("some organization have not instantiated chaincode in the channel " + channelName);
				}
			}
			...
			for (int i = 0; i < peerWithOrgs.length; i++) {
				//CommonResult commonResult = install_Upgrade_Chaincode(peerWithOrgs[i], channelName, chaincodePath, code.getChannelName(), chainCodeVersion);
				CommonResult commonResult = install_Upgrade_Chaincode(peerWithOrgs[i], channelName, chaincodePath, code.getChaincodeName(), chainCodeVersion);
			...
			System.out.println("upgrade2"+ code.getChaincodeName() +" "+codepath+" "+chainCodeVersion );
			//ChaincodeID chaincodeID = hyperledgerConfiguration.getChaincodeId(code.getChannelName(),codepath, chainCodeVersion);
			ChaincodeID chaincodeID = hyperledgerConfiguration.getChaincodeId(code.getChaincodeName(),codepath, chainCodeVersion);
			channelService.reconstructChannel(peerWithOrgs, channelName, client);
			Channel channel = ChannelContext.get();
			...
			System.out.println(responses.size()+" "+ successful.size()+" "+failed.size());
			...
			System.out.println("upgrate proposal 2");
			System.out.println(channel.getPeers().size());
			TransactionEvent transactionEvent = channel.sendTransaction(responses,orderers).get(HyperledgerConfiguration.config.getTransactionWaitTime(), TimeUnit.SECONDS);
			if(transactionEvent.isValid()) {
				return CommonResult.success("Chaincode instantiated Successfully! The Txid is"+successful.iterator().next().getTransactionID());
	...
	private CommonResult install_Upgrade_Chaincode(String peerWithOrg, String channelName, String chaincodePath,
		...
		try {
			...
			System.out.println("install1 "+ chaincodeName +" "+codepath+" "+chainCodeVersion );
			// 改造
			ChaincodeID chaincodeID = hyperledgerConfiguration.getChaincodeId(chaincodeName, codepath,
					chainCodeVersion);
